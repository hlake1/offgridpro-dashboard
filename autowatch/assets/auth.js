/*!
 * Autowatch Dashboard — Auth gate
 * Simple client-side password protection using sessionStorage.
 *  - "Autowatch" → client role (default view)
 ...
 */
(function () {
  const STORAGE_KEY = 'autowatch_dashboard_auth';
  const PASSWORDS = {
    Autowatch: 'client',
    Tweak: 'admin',
  };
  ...
  function paintOverlay(onSubmit) {
    ...
    overlay.id = 'autowatch-auth-overlay';
    ...
    `;
    overlay.innerHTML = `
      <form id="autowatch-auth-form" style="max-width:380px;width:calc(100% - 3rem);background:#fff;padding:2rem 2.25rem;border:1px solid #e5e7eb;border-radius:6px;box-shadow:0 12px 30px rgba(0,0,0,0.06);">
        <div style="font-size:11px;letter-spacing:0.2em;color:#6b7280;text-transform:uppercase;font-weight:600;">Tweak Reporting</div>
        <div style="height:4px;background:linear-gradient(90deg,#1a1a1a 0%,#3a3f4a 60%,#8a94a6 100%);border-radius:2px;margin:0.75rem 0 1.25rem;"></div>
        <h1 style="font-size:1.5rem;font-weight:500;color:#111;margin:0 0 0.5rem;">Autowatch · Performance Reports</h1>
        <p style="font-size:0.875rem;color:#6b7280;margin:0 0 1.25rem;line-height:1.5;">Enter your access code to continue.</p>
        <input id="autowatch-auth-input" type="password" autofocus placeholder="Access code"
        ...
        <p id="autowatch-auth-err" style="color:#dc2626;font-size:0.8rem;margin:0.55rem 0 0;min-height:1em;"></p>
        <button type="submit" style="margin-top:0.9rem;width:100%;background:#0d0f14;color:#fff;padding:0.7rem;border:none;border-radius:4px;font-size:0.9rem;font-weight:500;letter-spacing:0.05em;cursor:pointer;">
          Continue
        </button>
      </form>
    `;
    document.body.appendChild(overlay);
    const form = overlay.querySelector('#autowatch-auth-form');
    const input = overlay.querySelector('#autowatch-auth-input');
    const err = overlay.querySelector('#autowatch-auth-err');
  ...
  function emitAuth(role) {
    document.dispatchEvent(new CustomEvent('autowatch:auth', { detail: { role } }));
    if (typeof window.AutowatchAuthReady === 'function') window.AutowatchAuthReady(role);
  }
  ...
  function ensureAuth(opts) {
    ...
    const role = getRole();
    if (role) {
      document.documentElement.classList.toggle('autowatch-admin', role === 'admin');
      queueMicrotask(() => emitAuth(role));
      if (typeof opts.onSuccess === 'function') opts.onSuccess(role);
      return role;
    }
    const guard = document.createElement('div');
    guard.id = 'autowatch-auth-guard';
